...
(function() {
    ...
    var SmtpClientHandler = function(config, session) {
        ...
        self.command_handlers.submit_sm = function (pdu) {
            self.session.send(pdu.response({message_id: "1"}));
            logger.info(self.system_id, self.session.session_id, 'keepalive');
        };
    ...
    var Server = function(config) {
        ...
        self.start = function() {
            ...
            self.amqp = new RabbitDriver(config, {name: config.server.mo_channel, client: {prefetch: config.server.mo_queue_length}}, true);
            self.amqp.init().then(function() {
                logger.info('initialized mo consumer');
                self.amqp.on('data', self.handle_mo);
            });
...
// });
            ...
            for(var i in self.clients) {
                var client = self.clients[i];
                if(client.authenticated && client.system_id == data.body.channel && (client.bind_type === bind_type.rx || client.bind_type == bind_type.trx)) {
            ...
            if(session_id === false) {
                ...
                var receiver = self.clients[session_id];
